...
{query}"""
    ...
    def pipe(self, prompt: str = None, **kwargs) -> Generator[str, None, None]:
        ...
        if not user_input:
            yield "Please provide a file hash to analyze or a question about the last analyzed file."
            return

        try:
            # If input looks like a hash, do a new VT query
            if self.looks_like_hash(user_input):
                ...
                if vt_response:
                    ...
                    prompt = self.create_llm_prompt("Provide a brief summary of this file analysis.", context)
                else:
                    yield f"No data found for hash: {user_input}"
                    return
            else:
                # Use existing context for questions
                context = self.get_context_from_vt_data(self.last_vt_data) if self.last_vt_data else ""
                if not context:
                    yield "Please provide a file hash first before asking questions."
                    return
                prompt = self.create_llm_prompt(user_input, context)
